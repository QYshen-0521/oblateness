...
从 calculate_noise 缓存表读取当前目标的 ``sigma_flux``，只使用 ``status == success`` 的目标；
生成 :math:`F_{\\mathrm{obs}}=F_{\\mathrm{inj}}+N(0,\\sigma)` 后计算
:math:`\\chi^2(f,\\theta)=\\sum_k((F_{\\mathrm{obl},k}-F_{\\mathrm{obs},k})/\\sigma)^2`。
...
# ---------------------------------------------------------------------------
PLANET_CONFIG: dict = {
    "csv_path": "data/nasa_archive/ps_tran_oblate_inputs_combined_tierab_20260416.csv",
    "row_index": 701,
...
GRID_CONFIG: dict = {
    ...
    "n_theta": 37,
    "figure_filename": "noised_chi2_grid_f_theta.png",
    "save_chi2_npy": True,
    "chi2_npy_filename": "noised_chi2_grid.npz",
}

NOISE_CONFIG: dict = {
    "cache_csv_path": "data/calculate_noise_cache_combined_tierab_20260416(1).csv",
...
def _load_success_noise_for_target(path: Path, target_name: str) -> dict[str, str]:
    """按目标名读取成功的噪声缓存行；不使用 row_index，避免缓存表和输入表错位。"""
    if not path.is_file():
    ...
    required = {"target", "status", "sigma_flux", "sigma_frac_ppm"}
    if not rows:
        raise ValueError(f"noise cache is empty: {path}")
    missing = required.difference(rows[0].keys())
    if missing:
        raise KeyError(f"noise cache missing columns: {sorted(missing)}")

    matches = [r for r in rows if r["target"].strip() == target_name]
    if not matches:
        raise ValueError(f"target {target_name} is not in noise cache: {path}")

    status = str(NOISE_CONFIG["required_status"])
    success_matches = [r for r in matches if r["status"].strip() == status]
    if not success_matches:
        available = ", ".join(sorted({r["status"].strip() for r in matches}))
        raise ValueError(f"target {target_name} has no {status!r} noise row; available status: {available}")

    row = success_matches[0]
    sigma_flux = float(row["sigma_flux"])
    if not np.isfinite(sigma_flux) or sigma_flux <= 0.0:
        raise ValueError(f"invalid sigma_flux for {target_name}: {row['sigma_flux']}")
...
def run_grid_inversion() -> dict:
    ...
    noise_row = _load_success_noise_for_target(root / str(NOISE_CONFIG["cache_csv_path"]), name)
    sigma_flux = float(noise_row["sigma_flux"])
    sigma_frac_ppm = float(noise_row["sigma_frac_ppm"])
    ...
    rng = np.random.default_rng(int(NOISE_CONFIG["random_seed"]))
    gaussian_noise = rng.normal(0.0, sigma_flux, size=f_inj_arr.shape)
    ...
    out = {
        ...
        "noise_cache_target": noise_row["target"],
        "noise_cache_row_index": int(noise_row["row_index"]),
        "sigma_flux": sigma_flux,
        "sigma_frac_ppm": sigma_frac_ppm,
        "noise_random_seed": int(NOISE_CONFIG["random_seed"]),
    }
    return out
...
def _save_figure(result: dict) -> Path:
    ...
    ax.set_title(f"{result['pl_name']} — noised $\\chi^2(f,\\theta)$ (JWST LD)")
...
def main() -> None:
    ...
    print("noise cache target:", result["noise_cache_target"], ", noise cache row_index =", result["noise_cache_row_index"])
    print("sigma_flux =", result["sigma_flux"], ", sigma_frac_ppm =", result["sigma_frac_ppm"])
    print("noise random seed =", result["noise_random_seed"])
    ...
    if GRID_CONFIG.get("save_chi2_npy"):
        ...
        np.savez_compressed(
            ...
            f_inj=result["f_inj"],
            f_obs=result["f_obs"],
            gaussian_noise=result["gaussian_noise"],
            sigma_flux=result["sigma_flux"],
            sigma_frac_ppm=result["sigma_frac_ppm"],
            noise_random_seed=result["noise_random_seed"],
            noise_cache_target=result["noise_cache_target"],
            noise_cache_row_index=result["noise_cache_row_index"],
        )
